...
* creat a net_service
**/
NET_API struct net_service*			net_create(int size);
/**
* close the service, only this function is not thread safe
**/
NET_API void						net_close(struct net_service* service);
/**
* do something like epoll_wait。you can call it on async thread
**/
NET_API int							net_wait(struct net_service* service, int timeout);
/**
* get events
**/
NET_API int							net_queue(struct net_service* service, struct net_event * events, int maxevents);
/**
* listen port，you can listen some port use the same net_service
**/
NET_API net_socket					net_listen(struct net_service* service, unsigned short port, unsigned short listen_cnt);
/**
* if listening socket have Eve_Accept event , call this function you can get a client net_socket
**/
NET_API net_socket					net_accept(struct net_service* service, net_socket nd);
/**
* connect remote service
**/
NET_API net_socket					net_connect(struct net_service* service, const char* ip, unsigned short port);
/**
*	cfg the net_socket
*	you can write/read net_socket after call @net_socket_cfg
*	you can set the net buff by config
**/
NET_API int							net_socket_cfg(struct net_service* service, net_socket nd, struct net_config* config);
/**
* close net_socket
* send_rest: if you want send all rest data in buff, set to 1
* 
**/
NET_API void						net_socket_close(struct net_service* service, net_socket nd, char send_rest);
/**
...
**/
NET_API int							net_socket_read(struct net_service* service, net_socket nd, void* buff, int usize);
/**
* write a msg to net_socket
* if the net_socket's write_buff not enough, it will send faild
**/
NET_API int							net_socket_write(struct net_service* service, net_socket nd, const void* buff, int usize);
/**
* ctl you param data, if data is null, will return the data current ctl
*/
NET_API param_type					net_socket_ctl(struct net_service* service, net_socket nd, param_type* data);
/**
*  current net_socket size, also include listening sockets, connecting sockets
*/
NET_API int							net_socket_size(struct net_service* service);
/**
* sleep
**/
NET_API void						net_service_sleep(long ms);
